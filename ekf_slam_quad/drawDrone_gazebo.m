...
%     w        = truth(6); 
    ...
    if t==0
        ...
        axis([ -15, 15, -7.5, 7.5, -1, 10]);
        
        grid on
        hold on
        
        for i=1:length(landmarks)
            if abs(landmarks(i,1))>6
                offset = 90;
            else
                offset = 0;
            end
...
%             plot3 (landmarks(i,2),landmarks(i,1),-landmarks(i,3),'or')
        ...
        fill([-14.5,14.5,14.5,-14.5],[-7,-7,7,7],[.8 .8 .8]);
%         set(h,'facealpha',.5)
...
%        landmark_est = landmarks(ll,:);
       landmark_est= [mu(3+2*ll-1),mu(3+2*ll),-5];
       c(:,:) = P(1,3+2*ll-1:3+2*ll,3+2*ll-1:3+2*ll);
        
%        c = 3*chol(c);
       [xx,yy,zz] = error_ellipse_2d(c,landmark_est,.25); 
%        xx = real(sqrt(xx));
%        yy = real(sqrt(yy));
%        zz = real(sqrt(zz));
       xdata = [xdata;NaN;xx];
