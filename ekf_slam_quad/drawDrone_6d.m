function drawDrone_6d(mu,t,landmarks,P,truth, past_xhat, past_truth,V,F,patchcolors)
...
%         estimate_handle = updateEst(mu,P,MM,landmarks,[]);
        plot_handle = updatePlot(past_xhat,[]);
%         plot_handle_truth = updatePlot_truth(past_truth,past_xhat,[]);
...
%         updateEst(mu,P,MM,landmarks,estimate_handle);
        plot_handle = updatePlot(past_xhat,plot_handle);
%         plot_handle_truth = updatePlot_truth(past_truth,past_xhat,plot_handle_truth);
...
%        c = 3*chol(c);
       [xx,yy,zz] = error_ellipse(c,landmark_est,.25); 
%        xx = real(sqrt(xx));
%        yy = real(sqrt(yy));
%        zz = real(sqrt(zz));
       xdata = [xdata;NaN;xx];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function handle = updatePlot(past_xhat,handle)
    xdata = past_xhat(:,1);
    ydata = past_xhat(:,2);
    zdata = past_xhat(:,3);
    if isempty(handle)
       handle = plot3(ydata,xdata,zdata,'.b');
    else
       handle.XData = ydata;
       handle.YData = xdata;
       handle.ZData = -zdata;
...
function handle = updatePlot_truth(past_truth,past_xhat,handle)
    xdata2 = past_truth(:,1);
    ydata2 = past_truth(:,2);
    zdata2 = past_truth(:,3);
    ...
    if isempty(handle)
       ...
       handle = [plot3(ydata2,xdata2,zdata2,'.r');plot3(xdata,ydata,zdata,'.b')];
%        handle2 = plot3(xdata,ydata,zdata,'.b');
    else
       handle(1).XData = ydata2;
       handle(1).YData = xdata2;
       handle(1).ZData = -zdata2;
       drawnow
       handle(2).XData = ydata;
       handle(2).YData = xdata;
       handle(2).ZData = -zdata;
       drawnow
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
